...
def clean_old_log():   
    try:
        if os.access(log_file_name, os.F_OK):  
...
def log_differ(
        ...
        def remove_path_content(content_with_path:str):
            result = content_with_path.split("/")[-1]
            return result
        ...
        try:
            with open(log_result_path1,'r',encoding='utf-8') as f1:
                ...
                for i1 in f1:
                    i1 = i1.replace('\\','/')
                    if i1.find("/") != -1:
                        i1 = remove_path_content(i1)
                    differ_list1.append(i1.lstrip())
            ...
            with open(log_result_path2,'r',encoding='utf-8') as f2:
                ...
                for i2 in f2:
                    i2 = i2.replace('\\','/')
                    if i2.find("/") != -1:
                        i2 = remove_path_content(i2)
                    differ_list2.append(i2.lstrip())
...
def write_config_json(
        path:str
): #如果没有config.json，那么这个函数会写一个标准json出来
...
def have_config():
    have_config= os.access("RN_log_filter_config.json", os.F_OK)
    if have_config:
